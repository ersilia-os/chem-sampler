...
def setup_data_and_model(params, model):
    ...
    # setup random seeds
    if params.val_seed is None:
        params.val_seed = params.seed
    ...
    if params.suppress_params is False:
        logger.info(
            "\n".join(
                "%s: %s" % (k, str(v)) for k, v in sorted(dict(vars(params)).items())
            )
        )
    ...
    if (
        ...
        and params.pretrained_property_embeddings_path
    ):
        model.binary_graph_property_embedding_layer.weight.data = torch.Tensor(
            np.load(params.pretrained_property_embeddings_path).T
        )
    ...
    if load_prefix is not None:
        if params.local_cpu is True:
            model.load_state_dict(
                torch.load(
                    os.path.join(exp_path, "{}_model".format(load_prefix)),
                    map_location="cpu",
                )
            )
        else:
            model.load_state_dict(
                torch.load(os.path.join(exp_path, "{}_model".format(load_prefix)))
            )
    ...
    if params.gen_num_samples > 0:
        generator = GraphGenerator(
            train_data,
            model,
            params.gen_random_init,
            params.gen_num_iters,
            params.gen_predict_deterministically,
            params.local_cpu,
        )
    ...
    if load_prefix is not None:
        opt.load_state_dict(
            torch.load(os.path.join(exp_path, "{}_opt_sd".format(load_prefix)))
        )

    lr = opt.param_groups[0]["lr"]
    lr_lambda = lambda iteration: lr_decay_multiplier(
        iteration,
        params.warm_up_iters,
        params.decay_start_iter,
        params.lr_decay_amount,
        params.lr_decay_frac,
        params.lr_decay_interval,
        params.min_lr,
        lr,
    )
...
def main(params):
    ...
    (
        ...
        total_iter,
        grad_accum_iters,
    ) = setup_data_and_model(params, model)

    for epoch in range(1, params.num_epochs + 1):
        print("Starting epoch {}".format(epoch), flush=True)
        for train_batch in train_loader:
            if total_iter % 100 == 0:
                print(total_iter, flush=True)
            if total_iter == params.max_steps:
                logger.info("Done training")
                break
            model.train()
            if hasattr(model, "seq_model"):
                model.seq_model.eval()
            ...
            _, batch_scores_graph, graph_property_scores = model(
            ...
            num_components = sum(
                [
                    (v != 0).sum().numpy()
                    for _, v in batch_target_type_graph.ndata.items()
                ]
            ) + sum(
                [
                    (v != 0).sum().numpy()
                    for _, v in batch_target_type_graph.edata.items()
                ]
            )
            ...
            metrics_to_print = []
            if (
                params.target_data_structs in ["nodes", "both", "random"]
                and batch_target_type_graph.ndata["node_type"].sum() > 0
            ):
                node_losses = {}
            ...
            if (
                params.target_data_structs in ["edges", "both", "random"]
                and batch_target_type_graph.edata["edge_type"].sum() > 0
            ):
                edge_losses = {}
            ...
            if params.predict_graph_properties is True:
                ...
                for name, scores in graph_property_scores.items():
                    graph_property_loss = normalise_loss(
                        F.mse_loss(scores, graph_properties[name], reduction="sum"),
                        len(scores),
                        num_components,
                        params.loss_normalisation_type,
                    )
            ...
            if params.suppress_train_log is False:
                ...
                log_string = (
                    "total_iter = {0:d}, loss = {1:.2f}".format(
                        total_iter, loss.cpu().item()
                    )
                    + log_string
                )
            ...
            if (
                params.target_frac_inc_after is not None
                and total_iter > 0
                and total_iter % params.target_frac_inc_after == 0
            ):
                train_data.node_target_frac = min(
                    train_data.node_target_frac + params.target_frac_inc_amount,
                    params.max_target_frac,
                )
                train_data.edge_target_frac = min(
                    train_data.edge_target_frac + params.target_frac_inc_amount,
                    params.max_target_frac,
                )
            ...
            if total_iter > 0 and total_iter % params.val_after == 0:
                ...
                node_property_losses = {
                    name: 0 for name in train_data.node_property_names
                }
                edge_property_losses = {
                    name: 0 for name in train_data.edge_property_names
                }
                node_property_num_components = {
                    name: 0 for name in train_data.node_property_names
                }
                edge_property_num_components = {
                    name: 0 for name in train_data.edge_property_names
                }
                graph_property_losses = {
                    name: 0 for name in params.graph_property_names
                }
                model.eval()
                set_seed_if(params.seed)
                for (
                    ...
                    if (
                        ...
                        for name, target_type in batch_target_type_graph.ndata.items():
                            iter_node_property_loss = (
                                F.cross_entropy(
                                    batch_scores_graph.ndata[name][
                                        target_type.numpy() != 0
                                    ],
                                    batch_orig_graph.ndata[name][
                                        target_type.numpy() != 0
                                    ],
                                    reduction="sum",
                                )
                            ...
                            node_property_num_components[name] += float(
                                (target_type != 0).sum()
                            )
                    ...
                    if (
                        ...
                        for name, target_type in batch_target_type_graph.edata.items():
                            iter_edge_property_loss = (
                                F.cross_entropy(
                                    batch_scores_graph.edata[name][
                                        target_type.numpy() != 0
                                    ],
                                    batch_orig_graph.edata[name][
                                        target_type.numpy() != 0
                                    ],
                                    reduction="sum",
                                )
                    ...
                    if params.predict_graph_properties is True:
                        for name, scores in graph_property_scores.items():
                            iter_graph_property_loss = (
                                F.mse_loss(
                                    scores, graph_properties[name], reduction="sum"
                                )
                                .cpu()
                                .item()
                            )
                ...
                (
                    avg_node_property_losses,
                    avg_edge_property_losses,
                    avg_graph_property_losses,
                ) = ({}, {}, {})
                if params.loss_normalisation_type == "by_total":
                    total_num_components = float(
                        sum(node_property_num_components.values())
                    ...
                    if params.predict_graph_properties is True:
                        for name, loss in graph_property_losses.items():
                            avg_graph_property_losses[name] = (
                                loss / total_num_components
                            )
                elif params.loss_normalisation_type == "by_component":
                    avg_val_loss = 0
                    if params.target_data_structs in ["nodes", "both", "random"]:
                        for name, loss in node_property_losses.items():
                            avg_node_property_losses[name] = (
                                loss / node_property_num_components[name]
                            )
                        avg_val_loss += sum(avg_node_property_losses.values())
                    if params.target_data_structs in ["edges", "both", "random"]:
                        for name, loss in edge_property_losses.items():
                            avg_edge_property_losses[name] = (
                                loss / edge_property_num_components[name]
                            )
                ...
                if params.gen_num_samples > 0:
                    calculate_gen_benchmarks(
                        generator, params.gen_num_samples, training_smiles, logger
                    )
