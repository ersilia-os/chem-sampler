...
def get_loss_weights(target_ds, ds_scores, equalise, local_cpu):
    if equalise is True:
        weight = torch.from_numpy(
            len(target_ds)
            / (np.bincount(target_ds.cpu(), minlength=len(ds_scores[0])) + 0.0001)
        ).float()
...
def normalise_loss(
    unnormalised_loss, num_ds_components, num_all_ds_components, loss_normalisation_type
):
...
def get_loss(
    ...
    if binary is True:
        loss = F.binary_cross_entropy(
            ds_scores, target_ds, weight=weight, reduction="sum"
        )
    else:
...
def reshape_and_get_targetcorresponding_outputs(
    edge_variables, node_variables, node_target_inds_vector, edge_target_coords_matrix
):
    reshaped_node_variables, reshaped_edge_variables = [], []
    for nv in node_variables:
        reshaped_node_variables.append(
            get_targetcorresponding_output(nv, node_target_inds_vector, nv.shape[-1])
        )
...
def get_only_target_info(
    ds_scores, original_ds_inds, ds_target_inds_vector, ds_num_classes, ds_target_types
):
    """Return information on only those data structures that are predicted"""
    ds_scores = get_targetcorresponding_output(
        ds_scores, ds_target_inds_vector, ds_num_classes
    )
...
def get_ds_stats(ds_scores, target_ds, ds_target_types):
    ...
    replace_ds_correct = torch.mul(
        (ds_preds == target_ds), (ds_target_types == 2)
    ).sum()
    ...
    mask_ds_acc = calculate_acc(
        mask_ds_correct, float((ds_target_types == 1).sum().item()), np.nan
    )
    replace_ds_acc = calculate_acc(
        replace_ds_correct, float((ds_target_types == 2).sum().item()), np.nan
    )
    recon_ds_acc = calculate_acc(
        recon_ds_correct, float((ds_target_types == 3).sum().item()), np.nan
    )
...
def get_majority_and_minority_stats(ds_preds, target_ds, majority_index):
    nonmajority_ds_correct = torch.mul(
        (ds_preds == target_ds), (target_ds != majority_index)
    ).sum()
    majority_ds_correct = torch.mul(
        (ds_preds == target_ds), (target_ds == majority_index)
    ).sum()
    nonmajority_ds_acc = calculate_acc(
        nonmajority_ds_correct,
        float((target_ds != majority_index).sum().item()),
        np.nan,
    )
    majority_ds_acc = calculate_acc(
        majority_ds_correct, float((target_ds == majority_index).sum().item()), np.nan
    )
    return (
        majority_ds_correct,
        nonmajority_ds_correct,
...
def get_result_distributions(preds, targets, target_types, desired_type, bin_length=4):
    ...
    correct_dist = np.bincount(
        targets[preds == targets].cpu().numpy(), minlength=bin_length
    )
    incorrect_pred_dist = np.bincount(
        preds[preds != targets].cpu().numpy(), minlength=bin_length
    )
    incorrect_true_dist = np.bincount(
        targets[preds != targets].cpu().numpy(), minlength=bin_length
    )
    return np.vstack((correct_dist, incorrect_pred_dist, incorrect_true_dist))


def get_all_result_distributions(
    preds, targets, target_types, desired_types, bin_length=5
):
    dists = []
    for t in desired_types:
        dists.append(
            get_result_distributions(preds, targets, target_types, t, bin_length)
        )
...
def lr_decay_multiplier(
    ...
    if iteration < warm_up_iters:
        ...
        if lr_decay_amount == 0:
            return max(
                lr_decay_frac ** ((iteration - decay_start_iter) // lr_decay_interval),
                min_lr / lr,
            )
        else:
            return max(
                ...
                - (
                    lr_decay_amount
                    * ((iteration - decay_start_iter) // lr_decay_interval)
                )
                / lr,
                min_lr / lr,
            )
...
def save_checkpoints(
    ...
    if no_save is False:
        if save_all is True:
            torch.save(
                model_state_dict, os.path.join(exp_path, "model_{}".format(total_iter))
            )
...
def calculate_gen_benchmarks(generator, gen_num_samples, training_smiles, logger):
    ...
    try:
        benchmark = KLDivBenchmark(
            number_samples=gen_num_samples, training_set=training_smiles
        )
...
def graph_to_mol(
    ...
    min_charge=-1,
    node_target_inds=None,
    edge_target_coords_mat=None,
    return_pos=False,
    ...
    for i, node_ind in enumerate(init_node_properties["node_type"]):
        if not 0 <= node_ind < len(symbol_list):
            continue
        atom = Chem.Atom(symbol_list[node_ind])
        if init_node_properties["charge"] is not None:
            atom.SetFormalCharge(
                init_node_properties["charge"][i].item() - abs(min_charge)
            )
        if init_node_properties["chirality"] is not None:
            chiral_tag = Chem.ChiralType.values[
                int(init_node_properties["chirality"][i])
            ]
    ...
    adj_mat_coords = list(
        zip(*np.triu_indices(len(init_node_properties["node_type"]), k=1))
    )
    edge_id = 0
    for i, j in adj_mat_coords:
        ...
        if edge in BOND_TYPES.keys():
            ...
            if (
                edge_target_coords_mat is not None
                and int(edge_target_coords_mat[i, j]) == 1
            ):
...
def calculate_kldiv(mean_q, logvar_q, mean_p=None, logvar_p=None):
    ...
    return (
        0.5
        * (
            logvar_p - logvar_q - 1 + var_q / var_p + (mean_q - mean_p) ** 2 / var_p
        ).sum()
    )
...
def copy_graph_remove_data(input_graph):
    ...
    for key in input_graph.ndata.keys():
        del output_graph.ndata[key]
    for key in input_graph.edata.keys():
        del output_graph.edata[key]
    return output_graph
