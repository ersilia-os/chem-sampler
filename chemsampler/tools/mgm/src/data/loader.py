...
class QM9DatasetAR(TorchDataset):
    ...
    def __getitem__(self, index):
        ...
        assert check_symmetric(
            unpadded_adj_mat.astype(int)
        )  # make sure bond matrix is symmetric
        assert (
            unpadded_adj_mat.shape[0] == num_nodes
            and unpadded_adj_mat.shape[1] == num_nodes
        )
        ...
        max_molecule_len = (
            3 + self.max_nodes + self.max_nodes * (self.max_nodes - 1) // 2
        )
        ...
        return [
            torch.LongTensor(np.array(molecule)),
            torch.LongTensor(np.array([molecule_len])),
        ]
...
class GraphDataset(TorchDataset):
    def __init__(self, node_type, edge_type, params, train=True):
        ...
        self.mask_round_func = (
            np.ceil if params.force_mask_predict is True else np.round
        )
        self.replace_round_func = (
            np.ceil if params.force_replace_predict is True else np.round
        )
        ...
        self.weighted_loss = (
            params.weighted_loss if hasattr(params, "weighted_loss") else False
        )
        ...
        if sparse.issparse(edge_type[0]):
            edge_type = [et.todense() for et in edge_type]
    ...
    def add_to_property_dict(
        self, dct, property_name, data, num_categories, mask_index, empty_index
    ):
        dct[property_name] = {
            "data": data,
    ...
    def get_loss_weights(self):
        ...
        for name, property_info in self.node_properties.items():
            ...
            for single_datapoint_property in property_info["data"]:
                if name == "charge":
                    single_datapoint_property = single_datapoint_property + abs(
                        self.min_charge
                    )
                node_property_counts[name] += np.bincount(
                    single_datapoint_property, minlength=ml
                )
        for name, property_info in self.edge_properties.items():
            edge_property_counts[name] = np.ones(property_info["num_categories"])
            ml = property_info["num_categories"]
            for single_datapoint_property in property_info["data"]:
                flattened_triu_property = (
                    np.triu(single_datapoint_property, k=1).flatten().astype(np.int)
                )
                edge_property_counts[name] += np.bincount(
                    flattened_triu_property, minlength=ml
                )

        node_property_weights = {
            name: torch.Tensor(counts.max() / counts)
            for name, counts in node_property_counts.items()
        }
        edge_property_weights = {
            name: torch.Tensor(counts.max() / counts)
    ...
    def get_target_inds(
        self, unpadded_inds, majority_index, num_components, component_type
    ):
        if self.debug_fixed:
            target_inds = np.array([0])
        elif (component_type == "node" and self.edges_only) or (
            component_type == "edge" and self.nodes_only
        ):
            ...
            if self.train is True:
                ...
                if self.target_frac_type == "fixed":
                    ...
                    target_frac = np.random.uniform(
                        0, getattr(self, "{}_target_frac".format(component_type))
                    )
            else:
            ...
            if (
                self.equalise is True
                and (unpadded_inds != majority_index).sum() != 0
                and (unpadded_inds == majority_index).sum() != 0
            ):
                ...
                num_target_nonmajority_components = max(
                    num_replacements - num_target_majority_components, 1
                )
                nonmajority_component_locs = np.where(unpadded_inds != majority_index)[
                    0
                ]
                replace = (
                    False
                    if num_target_majority_components <= len(majority_component_locs)
                    else True
                )
                target_inds = np.random.choice(
                    majority_component_locs,
                    num_target_majority_components,
                    replace=replace,
                )
                replace = (
                    False
                    if num_target_nonmajority_components
                    <= len(nonmajority_component_locs)
                    else True
                )
                target_inds = np.concatenate(
                    (
                        target_inds,
                        np.random.choice(
                            nonmajority_component_locs,
                ...
                target_inds = np.random.choice(
                    num_components,
                    int(np.ceil(num_components * target_frac)),
                    replace=False,
                )
        return target_inds
    ...
    def get_target_subtype_inds(self, target_inds, component_type):
        ...
        num_replaced_components = min(
            int(self.replace_round_func(replace_frac * len(target_inds))),
            len(target_inds) - num_masked_components,
        )
        replace_inds = shuffled_target_inds[
            num_masked_components : num_masked_components + num_replaced_components
        ]
        recon_inds = shuffled_target_inds[
            num_masked_components + num_replaced_components :
        ]
        return (
            mask_inds,
            replace_inds,
            recon_inds,
            num_masked_components,
            num_replaced_components,
        )
    ...
    def corrupt_nodes(
        ...
        node_target_inds = self.get_target_inds(
            unpadded_node_inds, 1, num_nodes, "node"
        )
        (
            node_mask_inds,
            node_replace_inds,
            ...
            num_masked_nodes,
            num_replaced_nodes,
        ) = self.get_target_subtype_inds(node_target_inds, "node")
        if num_masked_nodes > 0:
            # mask nodes
            init_nodes[node_mask_inds] = mask_index
            node_mask_predict_inds = np.random.choice(
                node_mask_inds,
                int(
                    self.mask_round_func(self.node_mask_predict_frac * num_masked_nodes)
                ),
                replace=False,
            )
        ...
        if num_replaced_nodes > 0:
            ...
            node_replace_predict_inds = np.random.choice(
                num_replaced_nodes,
                int(
                    self.replace_round_func(
    ...
    def corrupt_node_property(
        self, init, node_mask_inds, node_replace_inds, mask_index, num_categories
    ):
        ...
        if num_replaced_nodes > 0:
            ...
            replacement_values = np.random.randint(
                0, num_categories, size=num_replaced_nodes
            )
    ...
    def corrupt_edges(
        ...
        edge_target_inds = self.get_target_inds(
            unpadded_edge_inds, 0, num_edges, "edge"
        )
        (
            edge_mask_inds,
            edge_replace_inds,
            ...
            num_masked_edges,
            num_replaced_edges,
        ) = self.get_target_subtype_inds(edge_target_inds, "edge")

        if num_masked_edges > 0:
            # mask edges
            init_edges, edge_mask_coords = self.set_edge_values(
                init_edges,
                edge_mask_inds,
                edge_coords,
                self.edge_properties["edge_type"]["mask_index"],
            )
            mask_predict_edges = edge_mask_coords[
                np.random.choice(
                    len(edge_mask_coords),
                    int(
                        self.mask_round_func(
            ...
            if predict_edges is True and len(mask_predict_edges) > 0:
                mask_predict_edges_0, mask_predict_edges_1 = np.array(
                    list(zip(*mask_predict_edges))
                )
                edge_target_types[mask_predict_edges_0, mask_predict_edges_1] = 1

        if num_replaced_edges > 0:
            # replace edges
            replacement_edges = np.random.randint(
                0,
                self.edge_properties["edge_type"]["num_categories"],
                size=num_replaced_edges,
            )
            init_edges, edge_replace_coords = self.set_edge_values(
                init_edges, edge_replace_inds, edge_coords, replacement_edges
            )
            replace_predict_edges = edge_replace_coords[
                np.random.choice(
                    len(edge_replace_coords),
                    int(
                        self.replace_round_func(
            ...
            if predict_edges is True and len(replace_predict_edges) > 0:
                replace_predict_edges_0, replace_predict_edges_1 = np.array(
                    list(zip(*replace_predict_edges))
                )
    ...
    def corrupt_graph(
        self,
        unpadded_node_properties,
        init_node_properties,
        node_property_target_types,
        num_nodes,
        unpadded_edge_property_inds,
        ...
        for i, property_name in enumerate(unpadded_node_properties.keys()):
            if self.mask_independently is True or i == 0:
                (
                    ...
                    init_node_properties[property_name],
                    unpadded_node_properties[property_name],
                    node_property_target_types[property_name],
                    num_nodes,
                    predict_nodes,
                ...
                latest_node_property_target_types = node_property_target_types[
                    property_name
                ]
            else:
                self.corrupt_node_property(
                    init_node_properties[property_name],
                    node_mask_inds,
                    node_replace_inds,
                    self.node_properties[property_name]["mask_index"],
                ...
                node_property_target_types[property_name] = np.copy(
                    latest_node_property_target_types
                )
        ...
        for i, property_name in enumerate(unpadded_edge_property_inds.keys()):
            (
                ...
                init_edge_properties[property_name],
                edge_coords,
                unpadded_edge_property_inds[property_name],
                edge_property_target_types[property_name],
                num_edges,
                predict_edges,
            )
    ...
    def __getitem__(self, index):
        ...
        (
            unpadded_edge_properties,
            unpadded_edge_property_inds,
            init_edge_properties,
            orig_edge_properties,
            edge_property_target_types,
        ) = ({}, {}, {}, {}, {})
        for property_name, property_info in self.edge_properties.items():
            unpadded_data = property_info["data"][index]
        ...
        for k, v in self.graph_properties.items():
            if self.normalise_graph_properties is True:
                graph_properties[k] = torch.Tensor(
                    [
                        (v[index] - self.graph_property_stats[k]["mean"])
...
class QM9DatasetGraph(GraphDataset):
    def __init__(
        ...
        graph2binary_properties=None,
    ):
        node_type, edge_type, num_hs, charge, is_in_ring, is_aromatic, chirality = zip(
            *graph_infos
        )
        super().__init__(node_type, edge_type, params, train)
        self.max_hs = params.max_hs
        self.min_charge = params.min_charge
        self.max_charge = params.max_charge
        self.mask_all_ring_properties = params.mask_all_ring_properties

        self.node_property_names.extend(
            ["hydrogens", "charge", "is_in_ring", "is_aromatic", "chirality"]
        )
        ...
        if graph_properties != None:
            self.graph_properties["molwt"], self.graph_properties["logp"] = zip(
                *graph_properties
            )
            self.graph_property_stats = {
                name: {"mean": np.mean(data), "std": np.std(data)}
        ...
        self.add_to_property_dict(
            ...
            "is_in_ring",
            is_in_ring,
            num_is_in_ring_categories,
            is_in_ring_mask_index,
            0,
        )
        self.add_to_property_dict(
            self.node_properties,
            "is_aromatic",
            is_aromatic,
            num_is_aromatic_categories,
            is_aromatic_mask_index,
            0,
        )
        self.add_to_property_dict(
            self.node_properties,
            "chirality",
            chirality,
            num_chirality_categories,
            chirality_mask_index,
            0,
        )
...
class SizeSampler(Sampler):
    ...
    def __init__(self, data_source, edges_per_batch, shuffle=False):
        ...
        self.lengths = np.array(
            [
                len(nodes) ** 2
                for nodes in data_source.node_properties["node_type"]["data"]
            ]
        )
        self.indices = np.arange(len(data_source))
        self.indices = self.indices[
            np.argsort(self.lengths[self.indices], kind="mergesort")
        ]
        batch_ids = []
        current_batch_edges_per_graph, current_batch_id, current_batch_num_edges = (
            0,
            0,
            0,
        )
        for i, length in enumerate(self.lengths[self.indices]):
            ...
            if (
                ...
                or current_batch_num_edges > edges_per_batch
            ):
                current_batch_id += 1
                current_batch_edges_per_graph = length
        ...
        self.batches = [
            self.indices[bounds[i] : bounds[i + 1]] for i in range(len(bounds) - 1)
        ]
...
def load_graph_data(params, train_split=0.8):
    val_batch_size = (
        params.batch_size if params.val_batch_size is None else params.val_batch_size
    )
    val_edges_per_batch = (
        params.edges_per_batch
        if params.val_edges_per_batch is None
        else params.val_edges_per_batch
    )
    ...
    if params.graph_type == "QM9" or params.graph_type == "protein":
        ...
        val_data_path, val_graph_properties_path = (
            params.val_data_path,
            params.val_graph_properties_path,
        )

    train_graph_infos, val_graph_infos = load_train_val_data(
        params.data_path, val_data_path
    )
    train_graph_infos, filtered_train_indices = limited_node_graph_infos(
        train_graph_infos, params.max_nodes
    )
    val_graph_infos, filtered_val_indices = limited_node_graph_infos(
        val_graph_infos, params.max_nodes
    )

    def load_and_split_from_indices(
        train_properties_path, val_properties_path, condition
    ):
        """
        ...
        """
        if condition:
            train_infos, val_infos = load_train_val_data(
                train_properties_path, val_properties_path
            )
    ...
    train_graph_property_infos, val_graph_property_infos = load_and_split_from_indices(
        params.graph_properties_path,
        val_graph_properties_path,
        len(params.graph_property_names) > 0,
    )

    (
        train_graph2binary_properties,
        val_graph2binary_properties,
    ) = load_and_split_from_indices(
        params.graph2binary_properties_path,
        params.val_graph2binary_properties_path,
    ...
    dataset_cls = (
        ProteinGraphDataset if params.graph_type == "protein" else QM9DatasetGraph
    )
    ...
    train_dataset = dataset_cls(
        ...
        params,
        train=True,
        graph_properties=train_graph_property_infos,
    ...
    val_dataset = dataset_cls(
        ...
        params,
        train=False,
        graph_properties=val_graph_property_infos,
        graph2binary_properties=val_graph2binary_properties,
    )
    if hasattr(params, "val_seed"):
        val_dataset.seed = params.val_seed
    if params.edges_per_batch > 0:
        train_batch_sampler = SizeSampler(
            train_dataset, params.edges_per_batch, params.shuffle
        )
        ...
        val_data_loader = DataLoader(
            val_dataset, batch_sampler=val_batch_sampler, collate_fn=graph_collate_fn
        )
    else:
        train_data_loader = DataLoader(
            train_dataset,
        ...
        val_data_loader = DataLoader(
            val_dataset, val_batch_size, collate_fn=graph_collate_fn
        )
...
def graph_collate_fn(batch):
    ...
    for element in batch:
        ...
        edge_property_target_types_transpose = {
            k: v.t() for k, v in edge_property_target_types.items()
        }
        ...
        init_graph.add_edges(
            *np.where(edge_mask),
            {k: v[np.where(edge_mask)] for k, v in init_edge_properties.items()}
        )
        orig_graph.add_edges(
            *np.where(edge_mask),
            {k: v[np.where(edge_mask)] for k, v in orig_edge_properties.items()}
        )
        target_type_graph.add_edges(
            *np.where(edge_mask),
            {k: v[np.where(edge_mask)] for k, v in edge_property_target_types.items()}
        )
        target_type_transpose_graph.add_edges(
            *np.where(edge_mask),
            {
                k: v[np.where(edge_mask)]
    ...
    return (
        batch_init_graph,
        batch_orig_graph,
        batch_target_type_graph,
        batch_target_type_transpose_graph,
        batch_graph_properties,
        batch_binary_graph_properties,
    )
...
def load_smiles_data(params):
    ...
    for splt in ["train", "valid", "test"]:
        ...
        if (
            ...
            and 1 < params.n_gpu_per_node <= data["mono_stream"][lang][splt].n_batches
        ):
        ...
        if splt == "train":
            data_iterator[splt] = data[splt].get_iterator(
                shuffle=True, group_by_size=True, n_sentences=-1
            )
        else:
