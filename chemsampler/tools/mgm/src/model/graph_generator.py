...
class MockGenerator(DistributionMatchingGenerator):
    def __init__(
        ...
        remove_non_novel=False,
    ):
        self.smiles_list = smiles_list
        if remove_non_novel is True:
            self.smiles_list = [
                s for s in self.smiles_list if s not in train_smiles_list
            ]
    ...
    def generate(self, number_samples):
        ...
        self.smiles_list = (
            self.smiles_list[number_samples:] + self.smiles_list[:number_samples]
        )
...
class GraphGenerator(DistributionMatchingGenerator):
    def __init__(
        ...
        local_cpu=False,
        cp_save_dir=None,
        set_seed_at_load_iter=False,
        graph_type="QM9",
        sample_uniformly=False,
        mask_comp_to_predict=False,
        maintain_minority_proportion=False,
        no_edge_present_type="learned",
        mask_independently=False,
        one_property_per_loop=False,
        checkpointing_period=1,
        save_period=1,
        evaluation_period=1,
        evaluate_finegrained=False,
        save_finegrained=False,
        variables_per_gibbs_iteration=1,
        top_k=-1,
        save_init=False,
        cond_property_values={},
    ):
        super().__init__()
        self.model = model
        ...
        self.cond_property_values = {
            k: float(v) for k, v in cond_property_values.items()
        }

    def generate(self, number_samples):
        load_path, load_iters = get_load_path(
            self.num_sampling_iters, self.num_argmax_iters, self.cp_save_dir
        )
        (
        ...
        for j in range(load_iters, self.num_iters):
            ...
            (
                ...
                all_node_masks,
                smiles_list,
            ) = self.carry_out_iteration(loader, use_argmax)
    ...
    def generate_with_evaluation(
        ...
        if self.save_init is True and self.random_init is True and load_iters == 0:
            ...
            for i in range(len(all_init_node_properties["node_type"])):
                mol = graph_to_mol(
                    {
                        k: v[i][: int(num_nodes[i])].astype(int)
                    ...
                    min_charge=self.train_data.min_charge,
        ...
        for j in tqdm(range(load_iters, self.num_iters)):
            ...
            if (j + 1 - load_iters) % self.evaluation_period == 0 or (
                ...
                json_output_path = os.path.join(
                    output_dir,
                    "distribution_results_{}_{}.json".format(
                        sampling_iters_completed, argmax_iters_completed
                    ),
                )
                ...
                if self.cond_property_values:
                    cond_json_output_path = os.path.join(
                        output_dir,
                        "cond_results_{}_{}.json".format(
                            sampling_iters_completed, argmax_iters_completed
                        ),
                    )
                    self.evaluate_cond_generation(
                        smiles_list[:num_samples_to_evaluate], cond_json_output_path
                    )

    def carry_out_iteration(self, loader, use_argmax):
        mols, smiles_list = [], []
        all_final_node_properties = {
            name: [] for name in self.train_data.node_property_names
        }
        all_final_edge_properties = {
            name: [] for name in self.train_data.edge_property_names
        }
        ...
        for (
            ...
            if self.local_cpu is False:
                ...
                if binary_graph_properties:
                    binary_graph_properties = binary_graph_properties.cuda()

            batch_init_graph = self.sample_simultaneously(
                batch_init_graph,
                batch_target_type_graph,
                batch_target_type_transpose_graph,
                graph_properties,
            ...
            self.append_and_convert_graphs(
                batch_init_graph,
                all_final_node_properties,
                all_final_edge_properties,
                all_final_node_masks,
    ...
    def get_all_init_variables(self, load_path, number_samples):
        if load_path is not None:
            ...
            all_node_masks = [
                (
                    node_type
                    != self.train_data.node_properties["node_type"]["empty_index"]
                )
            ...
            all_edge_masks = [
                (
                    edge_type
                    != self.train_data.edge_properties["edge_type"]["empty_index"]
                )
    ...
    def get_dataloader(
        ...
        if retrieve_train_graphs is False:
            for name, node_property in all_init_node_properties.items():
                ...
                for i, single_data_property in enumerate(node_property):
                    if name == "charge":
                        single_data_property -= abs(self.train_data.min_charge)
            ...
            for name, edge_property in all_init_edge_properties.items():
                ...
                for i, single_data_property in enumerate(edge_property):
                    data.append(
                        single_data_property[
                            : int(all_node_masks[i].sum()),
                            : int(all_node_masks[i].sum()),
                        ]
                    )
        ...
        for name, value in self.cond_property_values.items():
            gen_dataset.dataset.graph_properties[name] = (
                np.ones_like(gen_dataset.dataset.graph_properties[name]) * value
            )
        ...
        if self.edges_per_batch > 0:
            ...
            loader = DataLoader(
                gen_dataset, batch_sampler=batch_sampler, collate_fn=graph_collate_fn
            )
        else:
            loader = DataLoader(
                gen_dataset, batch_size=self.batch_size, collate_fn=graph_collate_fn
            )
        return loader

    def sample_simultaneously(
        self,
        batch_init_graph,
        batch_target_type_graph,
        batch_target_type_transpose_graph,
        graph_properties=None,
        binary_graph_properties=None,
        use_argmax=False,
    ):
        batch_preds_graph = copy_graph_remove_data(batch_init_graph)
        with torch.no_grad():
            _, batch_scores_graph, graph_property_scores = self.model_forward(
                batch_init_graph, graph_properties, binary_graph_properties
            )
        # This breaks symmetry for edge data
        batch_preds_graph = self.predict_from_scores(
            batch_scores_graph, batch_preds_graph, use_argmax
        )

        for name, target_type in batch_target_type_graph.ndata.items():
            batch_init_graph.ndata[name][
                target_type.numpy() != 0
            ] = batch_preds_graph.ndata[name][target_type.numpy() != 0]

        for name, target_type in batch_target_type_graph.edata.items():
            batch_init_graph.edata[name][
                target_type.numpy() != 0
            ] = batch_preds_graph.edata[name][target_type.numpy() != 0]
            batch_init_graph.edata[name][
                batch_target_type_transpose_graph.edata[name].numpy() != 0
            ] = batch_preds_graph.edata[name][
                batch_target_type_transpose_graph.edata[name].numpy() != 0
            ]

        return batch_init_graph

    def model_forward_mgm(
        self, batch_init_graph, graph_properties=None, binary_graph_properties=None
    ):
        return self.model(batch_init_graph, graph_properties, binary_graph_properties)

    def predict_from_scores(
        self, batch_scores_graph, batch_preds_graph, use_argmax=False
    ):
        for property_name, scores in batch_scores_graph.ndata.items():
            if use_argmax is True:
                batch_preds_graph.ndata[property_name] = torch.argmax(
                    F.softmax(scores, -1), dim=-1
                )
            else:
                if self.top_k > 0:
                    scores = filter_top_k(scores, self.top_k)
                batch_preds_graph.ndata[
                    property_name
                ] = torch.distributions.Categorical(F.softmax(scores, -1)).sample()
        for property_name, scores in batch_scores_graph.edata.items():
            if use_argmax is True:
                batch_preds_graph.edata[property_name] = torch.argmax(
                    F.softmax(scores, -1), dim=-1
                )
            else:
                if self.top_k > 0:
                    scores = filter_top_k(scores, self.top_k)
                batch_preds_graph.edata[
                    property_name
                ] = torch.distributions.Categorical(F.softmax(scores, -1)).sample()

        return batch_preds_graph

    def append_and_convert_graphs(
        self,
        batch_init_graph,
        all_final_node_properties,
        all_final_edge_properties,
        ...
        all_final_len = len(
            all_final_node_properties[list(all_final_node_properties.keys())[0]]
        )
        ...
        for i, num_nodes in enumerate(batch_init_graph.batch_num_nodes()):
            ...
            for name, property in batch_init_graph.ndata.items():
                all_final_node_properties[name].append(
                    property[node_start : node_start + num_nodes].numpy()
                )
            all_final_node_masks.append(
                np.ones(num_nodes)
            )  # redundant but needs to remain until node masks removed
            # from generation
            for name, property in batch_init_graph.edata.items():
                ...
                single_datapoint_fc_data[
                    batch_init_graph.edges()[0][
                        edge_start : edge_start + num_edges
                    ].numpy()
                    - node_start,
                    batch_init_graph.edges()[1][
                        edge_start : edge_start + num_edges
                    ].numpy()
                    - node_start,
                ] = property[edge_start : edge_start + num_edges].numpy()
                # force symmetry, which is broken earlier by sampling edge predictions
                single_datapoint_fc_data = np.triu(single_datapoint_fc_data) + np.tril(
                    single_datapoint_fc_data.T, -1
                )
            ...
            mol = graph_to_mol(
                {k: v[all_final_len + i] for k, v in all_final_node_properties.items()},
    ...
    def save_checkpoints(
        ...
        if self.cp_save_dir is not None:
            save_path = os.path.join(
                self.cp_save_dir,
                "gen_checkpoint_{}_{}.p".format(num_sampling_iters, num_argmax_iters),
            )
    ...
    def get_masked_variables(self, lengths, number_samples, pad=True):
        if pad is True:
            ...
            for name, property_info in self.train_data.node_properties.items():
                all_init_node_properties[name] = (
                    np.ones((number_samples, self.max_nodes))
                    * property_info["empty_index"]
                )
            for name, property_info in self.train_data.edge_properties.items():
                all_init_edge_properties[name] = (
                    np.ones((number_samples, self.max_nodes, self.max_nodes))
                    * property_info["empty_index"]
                )
        ...
        for sample_num, length in enumerate(lengths):
            if pad is False:
                for name, property_info in self.train_data.node_properties.items():
                    all_init_node_properties[name].append(
                        np.ones(length) * property_info["empty_index"]
                    )
                for name, property_info in self.train_data.edge_properties.items():
                    all_init_edge_properties[name].append(
                        np.ones((length, length)) * property_info["empty_index"]
                    )
            ...
            if self.random_init:
                for name, property_info in self.train_data.node_properties.items():
                    if self.sample_uniformly is True:
                        samples = np.random.randint(
                            0, property_info["num_categories"], size=length
                        )
                    else:
                        samples = (
                            torch.distributions.Categorical(
                                1 / self.train_data.node_property_weights[name]
                            )
                            .sample([length])
                            .numpy()
                        )
                ...
                for name, property_info in self.train_data.edge_properties.items():
                    if self.sample_uniformly is True:
                        samples = np.random.randint(
                            0,
                            property_info["num_categories"],
                            size=int(length * (length - 1) / 2),
                        )
                    else:
                        samples = (
                            torch.distributions.Categorical(
                                1 / self.train_data.edge_property_weights[name]
                            )
                            .sample([int(length * (length - 1) / 2)])
                            .numpy()
                        )
                    ...
                    for i in range(length):
                        ...
                        for j in range(i, length):
                            if i != j:
                                all_init_edge_properties[name][sample_num][
                                    i, j
                                ] = all_init_edge_properties[name][sample_num][
                                    j, i
                                ] = rand_edges.pop()
    ...
    def evaluate_cond_generation(self, smiles_list, json_output_path):
        ...
        graph_properties = calculate_graph_properties(
            valid_mols, self.cond_property_values.keys()
        )
        graph_property_stats = {
            name: {
                "mean": np.mean(graph_property),
                "median": np.median(graph_property),
...
def get_load_path(num_sampling_iters, num_argmax_iters, cp_save_dir):
    ...
    for fname in os.listdir(cp_save_dir):
        if "gen_checkpoint" not in fname:
            continue
    ...
    if (
        sampling_iters_to_load == num_sampling_iters
        and sampling_iters_to_load in all_cp_iters.keys()
    ):
        argmax_iters_to_load = min(
            max(all_cp_iters[sampling_iters_to_load]), num_argmax_iters
        )
    else:
...
def evaluate_uncond_generation(
    mock_generator,
    smiles_dataset_path,
    json_output_path,
    num_samples_to_evaluate,
    ...
    if evaluate_connected_only is True:
        mock_generator.smiles_list = [
            s for s in mock_generator.smiles_list if "." not in s
        ]
