...
class MPNN(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.node_mpnn = NODE_MPNN_DICT[node_mpnn_name](
            dim_h,
            use_layer_norm,
    ...
    def get_batch_graph_func(self):
        return (
            self.batch_graph_from_data_fc
            if self.fully_connected is True
            else self.batch_graph_from_data_nonfc
        )

    def get_step_forward_func(self):
        return (
            self.mpnn_step_forward_fc
            if self.fully_connected is True
            else self.mpnn_step_forward_nonfc
        )
    ...
    def forward(self, batch_graph):
        if self.res_conn is True:
            old_nodes, old_edges = (
                batch_graph.ndata["nodes"],
                batch_graph.edata["edge_spans"],
            )
    ...
    def update_nodes(self, graphs, nodes, node_mask, adj_mat_inds, edges):
        if self.node_mpnn_name == "MAT":
            return self.node_mpnn(
                nodes,
                node_mask,
                adj_mat_inds,
                torch.zeros_like(adj_mat_inds).float(),
    ...
    def mpnn_step_forward_fc(
        self, graphs, nodes, edges, node_mask, edge_mask, adj_mat_inds, step_num
    ):
        # calculate new node representations
        nshp, eshp = nodes.shape, edges.shape
        graphs.ndata["nodes"] = nodes.reshape(nshp[0] * nshp[1], *nshp[2:])
        graphs.edata["edge_spans"] = edges.reshape(
            eshp[0] * eshp[1] * eshp[2], *eshp[3:]
        )
        new_edges = edges
        ...
        new_nodes = self.update_nodes(
            graphs, nodes, node_mask, adj_mat_inds, edges_for_node_update
        )
...
class EdgesOwnRepsMPNN(MPNN):
    ...
    def __init__(
        ...
        **kwargs
        ...
        network_components = [
            nn.Linear(dim_h + dim_h * dim_k, dim_h),
            nn.ReLU(),
            nn.Linear(dim_h, dim_h * dim_k),
        ]
        self.bound_edges = bound_edges
        if self.bound_edges is True:
            network_components.append(nn.Sigmoid())
    ...
    def update_edges(self, graph):
        ...
        edges = self.edge_transform_network(edge_node_cat).reshape(
            num_edges, dim_h, dim_k
        )
        del graph.edata["msg"]
        return edges
...
class EdgesFromNodesMPNN(MPNN):
    def __init__(
        ...
        **kwargs
        ...
        network_components = [
            nn.Linear(dim_h, int(dim_h / 2)),
            nn.ReLU(),
            nn.Linear(int(dim_h / 2), dim_h),
        ]
        self.bound_edges = bound_edges
        if self.bound_edges is True:
            network_components.append(nn.Sigmoid())
...
MPNN_DICT = {
    "EdgesFromNodesMPNN": EdgesFromNodesMPNN,
    "EdgesOwnRepsMPNN": EdgesOwnRepsMPNN,
}
