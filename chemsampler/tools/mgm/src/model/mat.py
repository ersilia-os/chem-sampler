...
def make_model(
    ...
    model = GraphTransformer(
        Encoder(
            EncoderLayer(d_model, c(attn), c(ff), dropout, scale_norm, use_adapter),
            N,
            scale_norm,
        ),
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, dropout, scale_norm, use_adapter):
        ...
        self.sublayer = clones(
            SublayerConnection(size, dropout, scale_norm, use_adapter), 2
        )
        self.size = size
...
def attention(
    ...
    if mask is not None:
        scores = scores.masked_fill(
            mask.unsqueeze(1).repeat(1, query.shape[1], query.shape[2], 1) == 0, -inf
        )
