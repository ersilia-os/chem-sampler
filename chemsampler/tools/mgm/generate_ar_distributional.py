...
def get_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Generate molecular smiles with transformer"
    )
    ...
    parser.add_argument(
        "--dist_file",
        type=str,
        help="Distribution file, file pointing to all available molecules",
    )
    parser.add_argument("--suite", type=str, default="v2", help="Suite of guacamol")
    parser.add_argument("--seed", type=int, default=0, help="random seed")

    parser.add_argument(
        "--batch_size", type=int, default=100, help="Number of sentences per batch"
    )
...
class SmilesTransformerGenerator(DistributionMatchingGenerator):
    ...
    def generate(self, number_samples):
        return generate_smiles(
            self.params, self.model, self.dico, number_samples, self.sample_temperature
        )
...
def main(params):
    ...
    generator = SmilesTransformerGenerator(
        params, dico, model, params.sample_temperature
    )
    json_file_path = os.path.join(
        params.output_dir, "distribution_learning_results.json"
    )
