...
def get_final_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="path to smiles strings for entire dataset",
    )
    parser.add_argument(
        "--gd_output_path",
        type=str,
        default="",
        help="directory path to save individual gd outputs",
    )
    parser.add_argument(
        "--json_output_path", default="", help="path to save overall results"
    )
    parser.add_argument(
        "--smiles_output_path", default="", help="path to save generated smiles strings"
    )
    parser.add_argument(
        "--output_dir",
        default="",
        help="directory to save results for checkpointed uncond generation",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="num iters between uncond checkpoints",
    )
    parser.add_argument(
        "--save_period",
        type=int,
        default=1,
        help="num iters between saving uncond generated smiles",
    )
    parser.add_argument(
        "--save_finegrained",
        action="store_true",
        help="save every iter for first 10 iters",
    )
    parser.add_argument(
        "--evaluation_period",
        type=int,
        default=1,
        help="num iters between uncond evaluations",
    )
    parser.add_argument(
        "--evaluate_finegrained",
        action="store_true",
        help="evaluate every iter for first 10 iters",
    )
    parser.add_argument(
        "--save_init",
        action="store_true",
        help="save smiles strings of initialised molecules "
    ...
    parser.add_argument(
        ...
        choices=["gibbs", "simultaneous"],
        default="simultaneous",
    )
    parser.add_argument(
        "--variables_per_gibbs_iteration",
        type=int,
        default=1,
        help="how many variables per graph to"
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="number of iterations for which to use Gibbs"
        "Sampling before using argmax",
    )
    parser.add_argument(
        "--top_k", type=int, default=-1, help="value of k for top-k sampling"
    )
    parser.add_argument(
        "--maintain_minority_proportion",
        action="store_true",
        help="force nodes initialised as non-carbon" "to remain non-carbon",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--mask_comp_to_predict",
        action="store_true",
        help="mask component that will be predicted in each Gibbs sampling iteration",
    )
    parser.add_argument(
        "--one_property_per_loop",
        action="store_true",
        help="only replace one node property after each"
    ...
    parser.add_argument(
        "--smiles_load_path",
        default=None,
        help="Path to smiles strings that have already been"
...
def setup_data_and_generator(params):
    ...
    output_dir = (
        os.path.dirname(params.json_output_path)
        if params.json_output_path
        else params.output_dir
    )
    ...
    if params.use_mock_generator is True:
        ...
        generator = GraphGenerator(
            train_data,
            model,
            params.generation_algorithm,
            params.random_init,
            params.num_iters,
            params.num_sampling_iters,
            params.batch_size,
            params.edges_per_batch,
            params.retrieve_train_graphs,
            params.local_cpu,
            params.cp_save_dir,
            params.set_seed_at_load_iter,
            params.graph_type,
            params.sample_uniformly,
            params.mask_comp_to_predict,
            params.maintain_minority_proportion,
            params.no_edge_present_type,
            params.mask_independently,
            params.one_property_per_loop,
            params.checkpointing_period,
            params.save_period,
            params.evaluation_period,
            params.evaluate_finegrained,
            params.save_finegrained,
            params.variables_per_gibbs_iteration,
            params.top_k,
            params.save_init,
            params.cond_property_values,
        )
...
def main(params):
    ...
    if params.use_mock_generator is True:
        evaluate_uncond_generation(
            generator,
            params.smiles_dataset_path,
            params.json_output_path,
            params.num_samples_to_evaluate,
            params.evaluate_connected_only,
        )
    else:
        _ = generator.generate_with_evaluation(
            params.num_samples_to_generate,
            params.smiles_dataset_path,
            params.output_dir,
            params.num_samples_to_evaluate,
            params.evaluate_connected_only,
