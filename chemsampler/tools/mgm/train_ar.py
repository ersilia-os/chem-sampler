...
def get_parser():
    ...
    parser.add_argument(
        "--val_data_path", type=str, default="", help="Validation data path for ChEMBL"
    )
    parser.add_argument(
        "--load_path", type=str, default=None, help="Path for loading checkpoint"
    )
    ...
    parser.add_argument(
        "--optimizer",
        type=str,
        default="adam,beta1=0.9,beta2=0.98,lr=0.0003",
    ...
    parser.add_argument("--bptt", type=int, default=1024, help="Sequence length")
    parser.add_argument(
        "--max_len",
        type=int,
        default=300,
    ...
    parser.add_argument(
        "--print_after", type=int, default=5, help="Print after certain number of iters"
    )
    ...
    parser.add_argument(
        "--max_steps", default=10e6, type=int, help="total number of steps"
    )
...
def calculate_loss(model, batch, params, get_scores=False):
    ...
    if get_scores:
        scores, loss = model(
            "predict", tensor=hid, pred_mask=pred_mask, y=y, get_scores=True
        )
        return scores, loss, y
    else:
        _, loss = model(
            "predict", tensor=hid, pred_mask=pred_mask, y=y, get_scores=False
        )
        return loss
...
def load_model(params, model, opt, logger):
    ...
    data = torch.load(
        params.load_path, map_location=lambda storage, loc: storage.cuda(-1)
    )
...
def main(params):
    ...
    if params.data_type == "ChEMBL":
        all_smiles_mols = open(
            os.path.join(params.data_path, "guacamol_v1_all.smiles"), "r"
        ).readlines()
    else:
        all_smiles_mols = open(
            os.path.join(params.data_path, "QM9_all.smiles"), "r"
        ).readlines()
    ...
    train_loader = rcycle(
        train_data.get_iterator(shuffle=True, group_by_size=True, n_sentences=-1)
    )
    ...
    for total_iter, train_batch in enumerate(train_loader):
        ...
        if total_iter > 0 and total_iter % params.epoch_size == (params.epoch_size - 1):
            ...
            for val_iter, val_batch in enumerate(val_loader):
                with torch.no_grad():
                    val_scores, val_loss, val_y = calculate_loss(
                        model, val_batch, params, get_scores=True
                    )
                # update stats
            ...
            try:
                benchmark = KLDivBenchmark(
                    number_samples=number_samples, training_set=all_smiles_mols
                )
