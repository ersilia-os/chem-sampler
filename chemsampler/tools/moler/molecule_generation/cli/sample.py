...
def save_samples(
    model_dir: str, num_samples: int, data_file: str, **model_kwargs,
) -> None:
    with load_model_from_directory(model_dir, **model_kwargs) as model:
        samples = model.sample(num_samples)
    dict = {"smiles": samples}
...
def get_argparser() -> argparse.ArgumentParser:
    parser = get_model_loading_parser(
        description="Sample SMILES strings from a trained model."
    )
    ...
    parser.add_argument(
        ...
        dest="beam_size",
        type=int,
        help="Beam size to use during decoding.",
    )
...
def run_from_args(args: argparse.Namespace) -> None:
    model_kwargs = {
        key: getattr(args, key) for key in ["beam_size", "seed", "num_workers"]
    }
