"""Tests for the jsonl MoLeR trace dataset class."""
...
@pytest.fixture
def loaded_dataset() -> JSONLMoLeRTraceDataset:
    params = JSONLMoLeRTraceDataset.get_default_hyperparameters()
    dataset = JSONLMoLeRTraceDataset(params)
...
def test_num_edge_types(loaded_dataset: JSONLMoLeRTraceDataset):
    # We expect 3 tied fwd/bkwd edge types and self loops:
    assert loaded_dataset.num_edge_types == 4


def test_node_feature_shape(loaded_dataset: JSONLMoLeRTraceDataset):
    # Fixed in the test dataset:
    assert loaded_dataset.node_feature_shape == (29,)


def test_load_preprocessed_data(loaded_dataset: JSONLMoLeRTraceDataset):
...
def test_batching(loaded_dataset: JSONLMoLeRTraceDataset):
    with loaded_dataset.get_context_managed_tf_dataset(DataFold.TEST) as test_data:
        ...
        # Check for some more MoLeR-specific keys.
        assert "correct_node_type_choices" in batch_labels
        assert "correct_first_node_type_choices" in batch_labels
