...
class Sparse_GP:
    ...
    def __init__(
        ...
        self.lsf = theano.shared(
            value=np.zeros(1).astype(theano.config.floatX)[0], name="lsf", borrow=True
        )
        self.z = theano.shared(
            value=np.zeros((n_inducing_points, input_d)).astype(theano.config.floatX),
            name="z",
            borrow=True,
        )
    ...
    def compute_output(self):
        ...
        if self.ignore_variances:
            ...
            self.B2 = T.outer(
                T.dot(self.KzzInv, self.meanCavity), T.dot(self.KzzInv, self.meanCavity)
            )
            ...
            if exact_output_vars:
                ...
                v2 = T.transpose(
                    T.sum(
                        T.sum(T.shape_padaxis(self.B, 2) * (self.psi2 - kk), 0),
                        0,
                        keepdims=True,
                    )
                )
                ...
                v1 -= T.dot(
                    self.EKxz * T.dot(self.EKxz, self.B2), T.ones_like(self.z[:, 0:1])
                )
                v2 -= T.dot(
                    self.Kxz * T.dot(self.Kxz, self.B), T.ones_like(self.z[:, 0:1])
                )
    ...
    def getLogNormalizerCavity(self):
        ...
        return (
            ...
            + casting(0.5)
            * T.dot(
                T.dot(T.transpose(self.meanCavity), self.covCavityInv), self.meanCavity
            )
        )
    ...
    def getLogNormalizerPrior(self):
        ...
        return casting(0.5 * self.n_inducing_points * np.log(2 * np.pi)) - casting(
            0.5
        ) * T.nlinalg.LogDetPSD()(self.KzzInv)
    ...
    def initialize(self):
        ...
        lls = np.log(
            0.5 * (np.median(dist[np.triu_indices(input_means.shape[0], 1)]) + 1e-3)
    ...
    def getLogZ(self):
        ...
        return (
            ...
            - 0.5 * (self.training_targets - self.output_means) ** 2 / self.output_vars
        )
    ...
    def compute_log_averaged_ei(self, x, X, randomness, incumbent):
        ...
        KxX = KxX - T.dot(
            T.dot(KxXz[0 : x.shape[0], :], KzzInv), KxXz[x.shape[0] : xX.shape[0], :].T
        )
        ...
        new_predictive_mean = T.tile(m_old_x, [1, randomness.shape[1]]) + T.dot(
        ...
        log_ei = T.log(
            (incumbent - new_predictive_mean) * ratio(s) + T.sqrt(new_predictive_var)
        ) + log_n_pdf(s)
