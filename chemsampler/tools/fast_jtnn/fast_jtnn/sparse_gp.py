...
def global_optimization(
    grid, lower, upper, function_grid, function_scalar, function_scalar_gradient
):
...
class SparseGP:
    ...
    def __init__(self, input_means, input_vars, training_targets, n_inducing_points):
        ...
        self.input_means = theano.shared(
            value=input_means.astype(theano.config.floatX), borrow=True, name="X"
        )
        self.input_vars = theano.shared(
            value=input_vars.astype(theano.config.floatX), borrow=True, name="X"
        )
    ...
    def predict(self, means_test, vars_test):
        ...
        if self.predict_function is None:
            ...
            self.predict_function = theano.function(
                [X, Z], predictions, givens={self.input_means: X, self.input_vars: Z}
            )
    ...
    def train_via_LBFGS(
        self, input_means, input_vars, training_targets, max_iterations=500
    ):
    ...
    def train_via_ADAM(
        ...
        for j in range(max_iterations):
            ...
            for i in range(n_batches):
                minibatch_data_means = input_means[
                    i * minibatch_size : min((i + 1) * minibatch_size, n_data_points), :
                ]
                minibatch_data_vars = input_vars[
                    i * minibatch_size : min((i + 1) * minibatch_size, n_data_points), :
                ]
                minibatch_targets = training_targets[
                    i * minibatch_size : min((i + 1) * minibatch_size, n_data_points), :
                ]
                ...
                print(
                    "Epoch: {}, Mini-batch: {} of {} - Energy: {} Time: {}".format(
                        j, i, n_batches, current_energy, elapsed_time
                    )
                )
                sys.stdout.flush()
    ...
    def batched_greedy_ei(self, q, lower, upper, n_samples=1):
        ...
        X = theano.shared(
            value=X_numpy.astype(theano.config.floatX), name="X", borrow=True
        )
        x = T.matrix("x", dtype=theano.config.floatX)
        ...
        for i in tqdm(range(1, q)):
            ...
            randomness_numpy = casting(
                0
                * np.random.randn(X_numpy.shape[0], n_samples).astype(
