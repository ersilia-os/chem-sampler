...
def main_bo(
    ...
    while iteration < iterations:
        ...
        next_inputs = sgp.batched_greedy_ei(
            sampling, np.min(X_train, 0), np.max(X_train, 0)
        )
        ...
        save_object(
            valid_smiles, os.path.join(save_dir, "valid_smiles{}.pkl".format(iteration))
        )
        ...
        save_object(scores, os.path.join(save_dir, "scores{}.pkl".format(iteration)))
