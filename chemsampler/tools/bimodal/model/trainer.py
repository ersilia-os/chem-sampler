...
class Trainer:
    ...
    def complete_run(self, stor_dir="../evaluation/", restart=False):
        ...
        for i in range(self._epochs):
            ...
            if restart:
                ...
                if (
                    check_model(
                        self._model_type, self._experiment_name, stor_dir, fold, i
                    )
                    and check_molecules(self._experiment_name, stor_dir, fold, i)
                    and tmp_stat_file.shape[0] > i
                ):
            ...
            statistic = self._model.train(
                self._data, label, epochs=1, batch_size=self._batch_size
            )
    ...
    def single_run(self, stor_dir="../evaluation/", restart=False):
        ...
        # Special preprocessing in the case of NADE
        if (
            self._model_type == "NADE" or self._model_type == "NADE_v2"
        ) and self._generation == "random":
            # First column stores correct SMILES and second column stores SMILES with missing values
            label = np.argmax(self._data[:, 0], axis=-1).astype(int)
            aug = self._data.shape[1] - 1
            label = np.repeat(label[:, np.newaxis, :], aug, axis=1)
        ...
        train_data, test_data, train_label, test_label = train_test_split(
            self._data, label, test_size=1.0 / 5, random_state=1, shuffle=True
        )
        # Build model
        self._model.build()
        ...
        for i in range(self._epochs):
            ...
            # to  (all_SMILES, molecular_size, encoding_size))
            statistic = self._model.train(
                train_data.reshape(-1, self._molecular_size, self._encoding_size),
            ...
            tot_loss.append(
                self._model.validate(
                    test_data.reshape(-1, self._molecular_size, self._encoding_size),
    ...
    def cross_validation(self, stor_dir="../evaluation/", restart=False):
        ...
        for train, test in self._kf.split(self._data):
            ...
            for i in range(self._epochs):
                ...
                # to  (all_SMILES, molecular_size, encoding_size))
                statistic = self._model.train(
                    self._data[train].reshape(
                        -1, self._molecular_size, self._encoding_size
                    ),
