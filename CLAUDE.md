# ChemSampler — Developer Guide

This is the developer guide for ChemSampler, a Python package built from the Ersilia Open Source Initiative's package template. The rules below apply to any code, docs, or release work in this repository.
...
- **Keep public APIs small.** ChemSampler is thought of as a simple API and CLI. Avoid over-parametrising function signatures; a few well-chosen arguments are better than many optional knobs.
...
```python
from chemsampler.utils.logging import logger
...
- **Use Click.** If the package exposes a CLI, build it with [Click](https://click.palletsprojects.com/), organised as `src/chemsampler/cli/commands/` with one file per command and a small `create_cli.py` that registers them. This mirrors [`ersilia-os/ersilia`](https://github.com/ersilia-os/ersilia).
...
- **Never use the package name as the H1 title.** For example, a package named `chemsampler` should not have `# chemsampler` at the top — write a short descriptive title instead.
